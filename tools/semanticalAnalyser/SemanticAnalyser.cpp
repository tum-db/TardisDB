...
scope_t construct_scope(Database& db, const tardisParser::SQLParserResult & result) {
    ...
    for (auto & rel_pair : result.relations) {
        if (!db.hasTable(rel_pair.first)) {
            throw tardisParser::incorrect_sql_error("unknown relation '" + rel_pair.first + "'");
        }
...
tardisParser::BindingAttribute fully_qualify(const tardisParser::BindingAttribute & current, const scope_t & scope) {
    ...
    if (it == scope.end()) {
        throw tardisParser::incorrect_sql_error("unknown column '" + current.second + "'");
    }
...
void validate_sql_statement(const scope_t & scope, Database& db, const tardisParser::SQLParserResult & result) {
    for (auto & attr_name : result.projections) {
        ...
        if (it == scope.end()) {
            throw tardisParser::incorrect_sql_error("unknown column '" + attr_name + "'");
        } else if (it->second.second == nullptr) {
            throw tardisParser::incorrect_sql_error("'" + attr_name + "' is ambiguous");
        }
    }
    ...
    for (auto & selection_pair : result.selections) {
        ...
        if (!in_scope(scope, binding_attr)) {
            throw tardisParser::incorrect_sql_error("unknown column '" + binding_attr.first + "." + binding_attr.second + "'");
        }
    }
    ...
    for (auto & join_pair : result.joinConditions) {
        ...
        if (!in_scope(scope, lhs)) {
            throw tardisParser::incorrect_sql_error("unknown column '" + lhs.first + "." + lhs.second + "'");
        }
        auto & rhs = join_pair.second;
        if (!in_scope(scope, rhs)) {
            throw tardisParser::incorrect_sql_error("unknown column '" + rhs.first + "." + rhs.second + "'");
        }
    }
}

void SemanticAnalyser::analyse_sql_statement(Database& db, tardisParser::SQLParserResult &result) {
